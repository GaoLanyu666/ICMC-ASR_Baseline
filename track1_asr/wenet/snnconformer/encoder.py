...
from wenet.transformer.convolution import SpikeConvolutionModule
...
class SpikeConformerBaseEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.embed = WENET_SUBSAMPLE_CLASSES[input_layer](
            ...
            if pos_enc_layer_type != 'rope_pos' else pos_emb_class(
                ...
                attention_heads, positional_dropout_rate), node, T)
    ...
    def forward(
        ...
        xs = xs.unsqueeze(0).repeat(self.T, 1, 1, 1)  # SNN
        masks = masks.unsqueeze(0).repeat(self.T, 1, 1, 1)  # SNN
        xs, pos_emb, masks = self.embed(xs, masks)
        #print("afterembed,xs:", xs)
        ...
        masks = masks.reshape(self.T,
                              masks.size(0) // self.T, masks.size(1),
                              masks.size(2)).contiguous()
        xs = xs.mean(0)  # SNN
        masks = masks[0]  # SNN
        return xs, masks
...
class SNNConformerEncoder(SpikeConformerBaseEncoder):
    ...
    def __init__(
        ...
        positional_dropout_rate: float = 0.1,
        attention_dropout_rate: float = 0.0,
        input_layer: str = "conv2d_spike_originmask",
        ...
        mlp_type: str = 'spikeconformer_position_wise_feed_forward',
        mlp_bias: bool = True,
        n_expert: int = 8,
        ...
        super().__init__(input_size, output_size, attention_heads,
                         ...
                         use_sdpa, layer_norm_type, norm_eps, node, T)
        ...
        convolution_layer_args = (output_size, cnn_module_kernel, activation,
                                  cnn_module_norm, causal, conv_bias, norm_eps,
                                  node, T)
...
#         # XXX:定义全局均值方差归一化层，用于对输入特征进行归一化
#         self.global_cmvn = global_cmvn

#         # XXX:定义输入嵌入层，将输入特征进行降采样处理，并加入位置编码信息
