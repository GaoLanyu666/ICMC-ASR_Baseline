...
def load_checkpoint(model: torch.nn.Module, path: str, local_rank: int = 0) -> dict:
    if torch.cuda.is_available():
        logging.info('Checkpoint: loading from checkpoint %s for GPU' % path)
        checkpoint = torch.load(path, map_location=lambda storage, loc: storage.cuda(local_rank))
    else:
