...
class SSA_ConvBN(nn.Module):
    ...
    def __init__(self,
        ...
        self.node = node
        if self.node == "plif":
            self.out_lif = MultiStepParametricLIFNode(detach_reset=True)
            self.q_lif = MultiStepParametricLIFNode(detach_reset=True)
            self.k_lif = MultiStepParametricLIFNode(detach_reset=True)
            self.v_lif = MultiStepParametricLIFNode(detach_reset=True)
            self.attn_lif = MultiStepParametricLIFNode(detach_reset=True,
                                                       v_threshold=0.5)
        elif self.node == "lif":
            ...
            self.attn_lif = MultiStepLIFNode(tau=2.0,
                                             v_threshold=0.5,
                                             detach_reset=True)
        else:
            self.out_lif = MultiStepLIFNode(tau=2.0, detach_reset=True)
        ...
        self.linear_q = nn.Conv1d(n_feat, self.inner_dim, kernel_size=1, stride=1, bias=query_bias)
        self.q_bn = torch.nn.BatchNorm1d(self.inner_dim)
        self.linear_k = nn.Conv1d(n_feat, self.inner_kv_dim, kernel_size=1, stride=1, bias=key_bias)
        self.k_bn = torch.nn.BatchNorm1d(self.inner_kv_dim)
        self.linear_v = nn.Conv1d(n_feat, self.inner_kv_dim, kernel_size=1, stride=1, bias=value_bias)
        self.v_bn = torch.nn.BatchNorm1d(self.inner_kv_dim)
        self.linear_out = nn.Conv1d(self.inner_dim, n_feat, kernel_size=1, stride=1, bias=query_bias)
        self.out_bn = torch.nn.BatchNorm1d(n_feat)
        self.dropout = nn.Dropout(p=dropout_rate)
    ...
    def _forward_linearx(self, name: str, x: torch.Tensor) -> torch.Tensor:
        assert x.ndim >= 3
        if name == 'query':
            x = self.linear_q(x.transpose(1, 2).contiguous())
            x = self.q_bn(x).transpose(
                1, 2).contiguous()
            x = self.q_lif(
            ...
            x = self.linear_k(x.transpose(1, 2).contiguous())
            x = self.k_bn(x).transpose(
                1, 2).contiguous()
            x = self.k_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
            ...
            assert name == 'value'
            x = self.linear_v(x.transpose(1, 2).contiguous())
            x = self.v_bn(x).transpose(
                1, 2).contiguous()
            x = self.v_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
    ...
    def forward_attention(
        ...
        if mask.size(-1) > 0:  # time2 > 0
            ...
            # mask, 0.0)  # (batch, head, time1, time2)
            attn = scores.masked_fill(mask, 0.0)  # (batch, head, time1, time2)
        # NOTE(xcsong): When will `if mask.size(2) > 0` be False?
        #   1. onnx(16/-1, -1/-1, 16/0)
        #   2. jit (16/-1, -1/-1, 16/0, 16/4)
        else:
        ...
        x = self.attn_lif(
            x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                      x.size(2)).contiguous())
        x = x.flatten(0, 1)
        x = self.linear_out(x.transpose(1, 2).contiguous())
        # print("attention",x.shape)
        # if len(x.shape) >= 4:  # SNN search
        # x = x.flatten(0, 1)
        x = self.out_bn(x).transpose(1,
                                                               2).contiguous()
        x = x.view(x_shape)  # (batch, ..., time1, d_model)
        # print("out",x.shape)
        x = self.out_lif(
            x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                      x.size(2)).contiguous()).flatten(0, 1)
        return x  # (Tbatch, ...,  time1, d_model)
    ...
    def forward(
        ...
        if not self.use_sdpa:  # SNN
            scores = (
                q @ k.transpose(-2, -1)
            ) * self.scale  # torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_k)
            return self.forward_attention(v, scores, mask), new_cache
        else:
...
class RelPositionSSA_ConvBN(SSA_ConvBN):
    """Multi-Head Attention layer with relative position encoding.
    Paper: https://arxiv.org/abs/1901.02860
    Args:
        n_head (int): The number of heads.
    ...
    def __init__(self,
        ...
        super().__init__(n_head, n_feat, dropout_rate, query_bias, key_bias,
                         ...
                         node, T, scale)
        # linear transformation for positional encoding
        self.linear_pos = nn.Conv1d(n_feat, n_feat, kernel_size=1, stride=1, bias=False)
        self.bn_pos = torch.nn.BatchNorm1d(n_feat)  #
        if self.node == "plif":
            self.pos_lif = MultiStepParametricLIFNode(detach_reset=True)
        elif self.node == "lif":
            self.pos_lif = MultiStepLIFNode(tau=2.0, detach_reset=True)
        else:
            self.pos_lif = MultiStepLIFNode(tau=2.0, detach_reset=True)
        # these two learnable bias are used in matrix c and matrix d
        # as described in https://arxiv.org/abs/1901.02860 Section 3.3
    ...
    def forward(
        ...
        n_batch_pos = pos_emb.size(0)
        p = self.linear_pos(pos_emb.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        # p = self.bn_pos(p.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        # p = self.pos_lif(p.reshape(self.T, int(p.size(0)/self.T), p.size(1), p.size(2)).contiguous()) #
        ...
        if not self.use_sdpa:
            ...
            # print("matrix_ac.shape:", matrix_ac.shape)
            # print("matrix_bd.shape:", matrix_bd.shape)

            scores = (matrix_ac +
                      matrix_bd) * self.scale  # (batch, head, time1, time2)
...
class SSA_LN(nn.Module):
    ...
    def forward_attention(
        ...
        x_shape = x.size()[:-2] + torch.Size([self.h * self.d_k])
        x = x.view(x_shape)  # (batch, ..., time1, d_model)
        x_lif_shape = torch.Size([self.T, x.size(0) // self.T]) + x.size()[1:]
        #print("lif",x.shape)
        x = self.attn_lif(
            x.view(x_lif_shape).contiguous())
        x = x.flatten(0, 1)
        x = self.linear_out(x)
        # print("attention",x.shape)
        # if len(x.shape) >= 4:  # SNN search
        ...
        # print("out",x.shape)
        x = self.out_lif(
            x.view(x_lif_shape).contiguous()).flatten(0, 1)
        return x  # (Tbatch, ...,  time1, d_model)
...
class SSCA_LN(SSA_LN):
    ...
    def __init__(self,
                 ...
                 scale: float = 0.125):
        super().__init__(n_head, n_feat, dropout_rate, query_bias, key_bias,
                         value_bias, use_sdpa, n_kv_head, head_dim, node, T,
                         scale)
...
class RelPositionSSA_LN(SSA_LN):
    """Multi-Head Attention layer with relative position encoding.
    Paper: https://arxiv.org/abs/1901.02860
    Args:
        n_head (int): The number of heads.
    ...
    def __init__(self,
        ...
        # linear transformation for positional encoding
        self.linear_pos = nn.Linear(n_feat, n_feat, bias=False)
        self.bn_pos = torch.nn.LayerNorm(n_feat)  #
        if self.node == "plif":
            self.pos_lif = MultiStepParametricLIFNode(detach_reset=True)
        elif self.node == "lif":
            self.pos_lif = MultiStepLIFNode(tau=2.0, detach_reset=True)
...
class SSA_BN(nn.Module):
    ...
    def _forward_linearx(self, name: str, x: torch.Tensor) -> torch.Tensor:
        ...
        if name == 'query':
            x = self.linear_q(x)
            x = self.q_bn(x.transpose(1, 2).contiguous()).transpose(
                1, 2).contiguous()
            x = self.q_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
            ...
            x = self.linear_k(x)
            x = self.k_bn(x.transpose(1, 2).contiguous()).transpose(
                1, 2).contiguous()
            x = self.k_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
            ...
            assert name == 'value'
            x = self.linear_v(x)
            x = self.v_bn(x.transpose(1, 2).contiguous()).transpose(
                1, 2).contiguous()
            x = self.v_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
...
class RelPositionSSA_BN(SSA_BN):
    """Multi-Head Attention layer with relative position encoding.
    Paper: https://arxiv.org/abs/1901.02860
    Args:
        n_head (int): The number of heads.
...
class SSA(nn.Module):
    ...
    def forward_attention(
        ...
        x = x.view(x_shape)  # (batch, ..., time1, d_model)
        x = self.out_lif(x)
        return x  # (T, batch, ...,  time1, d_model)
    ...
    def forward(
        ...
        value: torch.Tensor,
        # mask: torch.Tensor = torch.ones((0, 0, 0), dtype=torch.bool),
        mask: torch.Tensor = torch.ones((0, 0, 0, 0), dtype=torch.bool),
        pos_emb: torch.Tensor = torch.empty(0),
        # cache: T_CACHE = (torch.zeros(0, 0, 0, 0), torch.zeros(0, 0, 0, 0)),
        cache: T_CACHE = (torch.zeros(0, 0, 0, 0,
                                      0), torch.zeros(0, 0, 0, 0, 0)),
    ) -> Tuple[torch.Tensor, T_CACHE]:
        """Compute scaled dot product attention.
...
class SSCA(SSA):
    ...
    def forward(
        ...
        pos_emb: torch.Tensor = torch.empty(0),
        # cache: T_CACHE = (torch.zeros((0, 0, 0, 0)), torch.zeros((0, 0, 0, 0)))
        cache: T_CACHE = (torch.zeros(
            (0, 0, 0, 0, 0)), torch.zeros((0, 0, 0, 0, 0)))
    ) -> Tuple[torch.Tensor, T_CACHE]:
        del pos_emb
        key_cache, value_cache = cache
        # assert key_cache.size(0) == value_cache.size(0)
        assert key_cache.size(1) == value_cache.size(1)
        # if key_cache.size(0) > 0:
        ...
        if B != k.size(1):
            ...
            B = k.size(1)
            # q = q.view(B, Beams, q.size(-3), q.size(-2), q.size(-1))
            q = q.view(self.T, B, Beams, q.size(-3), q.size(-2), q.size(-1))
            # k = k.unsqueeze(1)
            k = k.unsqueeze(2)
            # v = v.unsqueeze(1)
            v = v.unsqueeze(2)
        ...
        if query.size(1) != B:
            assert not self.training
            # output_shape = torch.Size([B * Beams]) + output.size()[2:]
            output_shape = torch.Size([self.T, B * Beams]) + output.size()[3:]
            output = output.view(output_shape)
        return output, new_cache
